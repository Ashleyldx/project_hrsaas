      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
           <UploadImg ref="employeesHeader" @onSuccess="headerSuccess"/>
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
      <UploadImg ref="employeesPic" @onSuccess="picSuccess"/>
        </el-form-item>
...
export default {
  ...
  methods: {
    async getPersonalDetail() {// 自定义
      // const res = await this.getPersonalDetail()
      this.formData = await getPersonalDetail(this.userId) // 函数名
      this.$refs.employeesPic.fileList = [{url: this.formData.staffPhoto}]
      // console.log(res);
    },
    async savePersonal() {
      // console.log(this.formData)
      // 判断一下是不是出于图片上传阶段 loading true 上传
    ...
    setImageUrl(staffPhoto) { // staffPhoto给loadingImg
